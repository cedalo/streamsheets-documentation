...
=TIMESCALE.CREATE_TABLE(Stream, Type, Name [, Timeout])
...
.. list-table::
   ...
   * - TableName
     - Defines which table to create.
   * - SchemaJSON
     - The timescale enforces the usage of a schema. Enter a JSON Range to setup the schema. Supported schema values can be NUMERIC, TEXT, TIMESTAMP,, TIMESTAMPTZ, DATE, INTEGER etc.
